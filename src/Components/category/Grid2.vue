...
export default {
  props: ["categoryName", "productsInNigeria", "productsIn1688", "categories" ],
  name: "Grid2",
  components: {
    Card,
    Breadc,
  ...
  // mounted(){
  //   // this.nigeriaProduct = this.productsInNigeria;
  //   // this.chinaProduct = this.productsIn1688;

  //   this.sortI(this.selectedCat._id)
  //   console.log(this.selectedCat);
  // },
  watch: {
    '$store.state.categories': function() {
      let cat = this.$store.state.categories.filter(c => c.categoryName.match(this.categoryName))
      this.sortI(cat[0]._id)
    }
  },
  computed: {
    ...mapState(["log"]),
    ...
    slicedNigeriaCards() {
      return this.nigeriaProduct.slice(0, this.showNigCards);
    },
    slicedCards1688() {
      return this.chinaProduct.slice(0, this.show1688Cards);
    }
  },
  methods: {
    incNigCardNumber() {
    ...
    sortI(id) {
      this.nigeriaProduct = this.productsInNigeria.filter(e =>
        e.categoryId.match(id)
      );
      this.chinaProduct = this.productsIn1688.filter(e =>
        e.categoryId.match(id));
    ...
    reSetNigeria() {
      return (this.nigeriaProduct = this.productsInNigeria);
    },
    reSet1688() {
      return (this.chinaProduct = this.productsIn1688);
    }
